...
- 锁定版本：`YWCREW_VERSION=v0.1.0 bash install.sh`（默认跟随 main 最新）
...
**被调的模型能看到我的整个项目吗？**
默认模式下，注入的上下文严格限于你圈定的文件白名单（secret guard 拦截凭据文件、路径逃逸、密钥特征），但被调模型运行在项目目录中，理论上可自行读取其他文件（各家的权限档是行为约束）。**敏感仓库请加 `"strict": true`**：任务会在只含白名单文件的影子目录中执行，被调模型物理上读不到项目的其他任何文件。

**模型返回的结论可信吗？**
每条 evidence（文件 + 行号）都由 worker 自动核验并标记 `verified`——文件不存在或行号越界会被如实标出。结论本身仍是建议性的，宿主智能体会对照代码库复核后再采纳。
